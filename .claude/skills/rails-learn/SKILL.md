...
- Corrections: always worth capturing (even once, since the user had to fix
  Claude).
- Error resolutions: worth capturing if the same class of error appeared 2+
  times.
- Workflows: worth capturing if repeated 3+ times.
- Conventions: worth capturing if they reflect a deliberate, non-default
  choice.
