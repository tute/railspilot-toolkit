...
RailsPilot is an AI-augmented Rails development service run through Your Company
LLC. The service delivers up to 12 production-ready features per month at
$X,XXX/month. Staff Engineers review all AI-generated code.
...
```bash
# Author filter: only the configured git user (e.g. e.g. Your Name)
...
- Professional but warm. For example for ClientA or ClientB projects, the client is also a colleague, so the email is a business document.
